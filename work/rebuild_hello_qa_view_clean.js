...
const nav = displayCategories.map((cat) => `<div class="nav-row" style="--cat:#${htmlEscape(cat.color)}"><button class="cat nav-filter" data-category="${htmlEscape(cat.id)}"><span></span><strong>${htmlEscape(cat.title)}</strong><b>${(byCategory.get(cat.id) || []).length}</b></button><a class="group-page" href="${htmlEscape(groupFileName(cat))}" title="${htmlEscape(cat.title)}の個別ページ">開く</a></div>`).join('\n');
...
for (const cat of displayCategories) {
  ...
  fs.writeFileSync(path.join(OUT, groupFileName(cat)), page({
    ...
    meta: `<span class="pill">質問 ${items.length}件</span><span class="pill">回答 ${answers}件</span><span class="pill">OG統合回答 ${moved}件</span><span class="pill"><a href="index.html">全体へ戻る</a></span>`,
    buttons: groupLinks,
    navHtml: '',

...
if (fs.existsSync(MANIFEST_PATH)) {
  try {
    ...
    for (const item of previous.items || []) {
      const order = item.order || Number((path.basename(item.file_name || item.local_path).match(/^(\d+)/) || [])[1] || 0);
      previousLinks.set(`${item.program}\u0000${order}`, item);
    }
  } catch {
...
function parseFile(file) {
  ...
  const date = (title.match(/【([^】]+)】/) || [])[1] || (title.match(/(\d{4}年\d{1,2}月\d{1,2}日配信)/) || [])[1] || (title.match(/(\d{1,2}月\d{1,2}日配信)/) || [])[1] || '';
  const host = (title.match(/ - ([^-[]+)\s*\[mid\d+\]$/i) || [])[1]?.trim() || (folder.includes('ろこの部屋') ? '筒井澪心' : '');
  const guestMatches = [...title.matchAll(/ゲスト[：:_]\s*([^【\-\[]+)/g)];
  const guest = guestMatches.at(-1)?.[1]?.trim() || '';
  const previous = previousLinks.get(`${folder}\u0000${order}`) || {};
  let linkedMid = '';
  if (previous.audio_url) {
    try {
      linkedMid = decodeURIComponent(new URL(previous.audio_url).pathname.split('/').pop() || '').match(/mid(\d+)/i)?.[1] || '';
    } catch {}
  }
  const mid = (title.match(/\[mid(\d+)\]/i) || [])[1] || previous.mid || linkedMid;
...
const programDirectory = programs.map((program) => {
  const rows = episodes.filter((tracks) => tracks[0].program === program);
  const files = rows.reduce((sum, tracks) => sum + tracks.length, 0);
  const size = rows.reduce((sum, tracks) => sum + tracks.reduce((partSum, item) => partSum + item.size, 0), 0);
  const title = program.replace(/^オリジナル番組[「"]?/, '').replace(/[」"]$/, '');
  return `<button class="program-choice" type="button" data-select-program="${esc(program)}"><strong>${esc(title)}</strong><span>${rows.length}回 · ${files}ファイル · ${(size / 1024 / 1024 / 1024).toFixed(1)}GB</span></button>`;
}).join('\n');
...
<style>
.program-directory{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,260px),1fr));gap:10px;align-content:start}.directory-heading{grid-column:1/-1;margin:0 0 2px}.directory-heading h2{margin:0;font-size:20px}.directory-heading p{margin:3px 0 0;color:var(--sub);font-size:13px}.program-choice{display:grid;gap:5px;min-height:88px;padding:14px;text-align:left;border:1px solid var(--line);border-radius:8px;background:#fff;color:var(--ink);cursor:pointer}.program-choice:hover{border-color:var(--accent);background:var(--accent-soft)}.program-choice strong{font-size:16px}.program-choice span{color:var(--sub);font-size:13px}
</style>
</head>
<body>
...
<section class="global-player">
  <div class="global-inner">
    ...
    <details class="history-panel">
      <summary>再生履歴</summary>
      <div id="historyList" class="history-list"></div>
      <button id="historyClear" class="history-clear" type="button">履歴を消す</button>
    </details>
  </div>
</section>
<main>
...
<section id="list"><div id="programDirectory" class="program-directory"><header class="directory-heading"><h2>番組一覧</h2><p>${programs.length}番組から選択</p></header>${programDirectory}</div>${rows}</section>
</main>
<script>
const q=document.getElementById('q'), program=document.getElementById('program'), clear=document.getElementById('clear'), summary=document.getElementById('summary'), jumpList=document.getElementById('jumpList'), programDirectory=document.getElementById('programDirectory');
...
const historyKey='hpm-radio-history-v1';
...
function renderHistory(){
  ...
  for(const item of items){
    ...
    button.querySelector('small').textContent=item.playedAt||'';
    button.addEventListener('click',()=>{
      const track=allTracks.find(candidate=>candidate.dataset.url===item.url);
      if(track) activateTrack(track,true);
    });
...
function saveHistory(button){
  ...
  const item={url:button.dataset.url,title,playedAt:new Date().toLocaleString('ja-JP',{month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'})};
  const next=[item,...readHistory().filter(history=>history.url!==item.url)];
  writeHistory(next);
  renderHistory();
}
function trackIndex(){
...
if(historyClear) historyClear.addEventListener('click',()=>{writeHistory([]);renderHistory();});
renderHistory();
updateNav();
if(media){
...
function apply(){
  ...
  const showDirectory=!selected&&!term;
  programDirectory.classList.toggle('hidden',!showDirectory);
  let shown=0;
  const visible=[];
  for(const card of cards){
    const okProgram=!selected||card.dataset.program===selected;
    const okText=!term||card.dataset.search.toLowerCase().includes(term);
    const ok=!showDirectory&&okProgram&&okText;
  ...
  summary.textContent=showDirectory?programs.length+'番組':shown+' / '+cards.length+'本';
  renderJumpList(visible);
}
programDirectory.addEventListener('click',event=>{
  const button=event.target.closest('[data-select-program]');
  if(!button) return;
  program.value=button.dataset.selectProgram;
  apply();
  syncUrl();

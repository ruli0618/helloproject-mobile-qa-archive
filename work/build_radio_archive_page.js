...
const rows = episodes.map((tracks) => {
  ...
  <div class="player"${playable.length ? '' : ' data-empty="true"'}>
    ${playable.length ? `<video controls playsinline preload="metadata" src="${esc(playable[0].audio_url)}"></video><div class="now-playing">再生中: <span>${esc(playable[0].title)}</span><a class="open-current" href="${esc(playable[0].audio_url)}" target="_blank" rel="noopener noreferrer">直接開く</a></div>` : '<span>音源アップロード後にここで再生できます</span>'}
  </div>

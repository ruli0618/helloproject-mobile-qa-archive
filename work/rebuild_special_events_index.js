...
const lightbox=document.getElementById('lightbox'), lightboxImage=document.getElementById('lightboxImage'), lightboxTitle=document.getElementById('lightboxTitle'), lightboxClose=document.getElementById('lightboxClose'), rotateLeft=document.getElementById('rotateLeft'), rotateRight=document.getElementById('rotateRight');
let lightboxRotation=0;
function updateRotation(){const sideways=Math.abs(lightboxRotation % 180) === 90; lightboxImage.style.transform='rotate('+lightboxRotation+'deg)'; lightboxImage.style.maxWidth=sideways ? '86vh' : '100%'; lightboxImage.style.maxHeight=sideways ? '96vw' : '86vh';}
function openLightbox(link){lightboxRotation=0; updateRotation(); lightboxImage.src=link.href; lightboxImage.alt=link.dataset.title || ''; lightboxTitle.textContent=link.dataset.title || ''; lightbox.classList.add('open'); lightbox.setAttribute('aria-hidden','false');}
...
rotateRight.addEventListener('click',()=>{lightboxRotation+=90; updateRotation();});

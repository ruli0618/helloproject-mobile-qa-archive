...
<nav>
  <div class="controls">
    ...
    <select id="categorySelect" aria-label="ツアーで絞り込み">
      <option value="all">すべて (${entries.length})</option>
      ${categories.map((category) => `<option value="${esc(category.category_id)}">${esc(category.category_title)} (${(byCategory.get(category.category_id) || []).length})</option>`).join('\n')}
    </select>
  ...
  <details class="compact-nav">
    <summary>ツアーボタン一覧</summary>
    <div class="compact-nav-body">${nav}</div>
  </details>
</nav>
...
categorySelect.addEventListener('change', () => { active = categorySelect.value; apply(); if(active !== 'all') document.getElementById('tour-' + CSS.escape(active))?.scrollIntoView({block:'start'}); });
